...
public class XCheck {
    ...
    public static boolean checkMaNV(JTextField txt) {
        ...
        String rgx = "[a-zA-Z0-9]{1,20}";
        if (id.matches(rgx)) {
            return true;
        } else {
            txt.setBackground(pink);
            alert(txt.getRootPane(), txt.getName() + " phải có 1-20 kí tự\nchữ hoa, thường không dấu hoặc số.");
            return false;
        }
    }
    ...
    public static boolean checkMa(JTextField txt) {
        ...
        if (id.matches(rgx)) {
            ...
            txt.setBackground(pink);
            alert(txt.getRootPane(), txt.getName() + " phải có đúng 7 kí tự\nchữ thường, chữ hoa hoặc số");
            return false;
        }
    }
    ...
    public static boolean checkPass(JPasswordField txt) {
        ...
        if (txt.getPassword().length > 2 && txt.getPassword().length < 17) {
            ...
            txt.setBackground(pink);
            alert(txt.getRootPane(), txt.getName() + " phải có từ 3-16 kí tự.");
            return false;
        }
    }
...
//        }
        if (isValidDate(id)) {
            ...
            txt.setBackground(pink);
            alert(txt.getRootPane(), txt.getName() + "Không đúng định dạng dd/MM/yyyy");
...
//        }
        if (isValidDate(id)) {
            ...
            alert(null, "Không đúng định dạng dd/MM/yyyy");
    ...
    public static boolean checkMoTa(JTextArea txt) {
        ...
        if (id.matches(rgx)) {
            ...
            txt.setBackground(pink);
            alert(txt.getRootPane(), txt.getName() + " phải từ 3-255 kí tự.");
            return false;
        }
    }
